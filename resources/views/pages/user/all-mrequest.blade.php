        ...
        <div class="col-lg-4">
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb breadcrumb-title">
                    ...
                    <li class="breadcrumb-item">
                        <a href="{{ url('frequests') }}">Meat Requests</a>
                    </li>
                    <li class="breadcrumb-item">
                              ...
                              <tbody>
                                @if ($mrequests)
                                    @foreach ($mrequests as $mrequest )
                                        <tr>
                                            <td>{{ $mrequest->request }}</td>
                                            <td>{{ $mrequest->meatallocation }}</td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <span
                                                        ...
                                                        >{{ $mrequest->status }}</span
                                                    >
                                            </td>
                                            <td style="white-space: nowrap;width:20%;">
                                                <form method="POST" action="{{ route('frequests.destroy',$mrequest->id) }}" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="d-inline btn-sm btn btn-danger" data-toggle="tooltip" title="Delete Request"><i class="fa fa-trash-o"></i> Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
